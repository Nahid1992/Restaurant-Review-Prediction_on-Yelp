...
def kappa(test,pred,algo_name):
    path_name = "Results/10_Dataset_Run_48ATR_5CLASS.txt"
...
def showAttributes(xTicks,selectedSum,selectedList):
    plt.figure(1) 
    plt.title('Number of Attributes: ' + str(len(selectedList)))
    plt.ylabel('Attributes')
    plt.xlabel('Count')
    plt.barh(xTicks,selectedSum)
    plt.yticks(xTicks,selectedList)  
    plt.tight_layout()
    path_name = "Results/10_Dataset_Run_48ATR_2CLASS"
...
if __name__ == '__main__':
    ...
    print "10_Dataset_Run_48ATR_2CLASS => Old Set + No Cut"
...
#    db = db[ATR_ALL]
    ...
    for col in db.columns:
        ...
        if atr_name == 'attributes':            
            ...
            count = count + 1   
            selectedList.append(col.split('attributes.')[-1])
            selectedSum.append(SUM)
            xTicks.append(count_index)
            count_index = count_index+1
            
            
    print('Attr. Count = ' + str(48))    
...
#    Multi_Nomial_NB(X_train,y_train,X_test,y_test,num_class)
    ...
    result_DF.to_csv('Results/10_Table_Result_48ATR_5CLASS.csv')